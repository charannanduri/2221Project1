...
public final class CryptoUtilities {
    ...
    public static void powerMod(NaturalNumber n, NaturalNumber p,
        ...
        if (n.isZero() && p.isZero()) {
            NaturalNumber r = one.divide(m);

            n.copyFrom(r);

        } else if (!p.isZero() && n.isZero()) {
            n.clear();
        } else {
            NaturalNumber r = n.divide(m);
            n.copyFrom(r);
            if (!p.isZero() && isEven(p)) {
                p.divide(two);
                powerMod(n, p, m);
                temp.copyFrom(n);
                p.multiply(two);
    ...
    public static boolean isWitnessToCompositeness(NaturalNumber w,
        ...
        NaturalNumber two = new NaturalNumber2(2);
        NaturalNumber one = new NaturalNumber2(1);
        NaturalNumber wTemp = new NaturalNumber2();
        ...
        n.subtract(one);
        powerMod(wTemp, n, nTemp);
        if (wTemp.compareTo(one) != 0) {
            isWit = true;
        }
    ...
    public static boolean isPrime2(NaturalNumber n) {
        ...
        if (n.compareTo(new NaturalNumber2(THREE)) <= 0) {
            ...
            for (int i = 0; i < 25 && !p; i++) {
                ...
                n.add(four);
                rand.add(two);
                p = !isWitnessToCompositeness(rand, n);
            }
        }
