...
public final class StringReassembly {
    ...
    public static String combination(String str1, String str2, int overlap) {
        ...
        // TODO: fill in body
        int len = str1.length();
        String firstHalf = str1.substring(0, (len - overlap));
        String together = firstHalf.concat(str2);
        /*
    ...
    public static void addToSetAvoidingSubstrings(Set<String> strSet,
        ...
        Set<String> subStr = strSet.newInstance();
        for (String item : strSet) {
            isSubStr = isSubStr & item.indexOf(str) > 0;
        }
        if (isSubStr != true) {
            for (String item : strSet) {
    ...
    public static Set<String> linesFromInput(SimpleReader input) {
        ...
        Set<String> inStr = new Set1L<String>();
        String str = input.nextLine();
        while (!input.atEOS()) {
            addToSetAvoidingSubstrings(inStr, str);
            str = input.nextLine();
        }
    ...
    public static void printWithLineSeparators(String text, SimpleWriter out) {
        ...
        for (int i = 0; i < len; i++) {
            if (temp.charAt(i) == '~') {
                String half = temp.substring(0, i);
                String otherHalf = temp.substring(i + 1);
                temp = half + "\n" + otherHalf;
            }
        }
        out.print(temp);
